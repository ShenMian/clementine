...
public:
			...
			: Application("Pong")
	{
		...
		ball.addComponent<Rigidbody>().velocity = {0.5f, 0.05f}; // 为 ball 创建一个 Rigidbody 组件, 并设置初速度
		ball.getComponent<Transform>().position = {10, 12};      // 设置 ball 的位置, Transform 组件默认存在
		...
		bat2.addComponent<Script>().onUpdate = [&](float dt) {
			...
			batBody.velocity = Vec2(0, ballPos.y - batCenter).normalize() * 0.2f;
		};
	}
