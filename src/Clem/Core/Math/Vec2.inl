...
template <class T>
Vec2<T>& Vec2<T>::rotate(const Vec2<float>& point, float angle)
{
	const float sinAngle = std::sin(angle);
	const float cosAngle = std::cos(angle);

	if(point == Vec2<float>(0, 0))
	{
...
template <class T>
Vec2<T>::operator Vec2<int>() const
{
	return Vec2<int>(static_cast<int>(x), static_cast<int>(y));
}

template <class T>
Vec2<T>::operator Vec2<float>() const
{
